...
# ── Task System (from s12, synced + s18 worktree field) ──
...
def scan_unclaimed_tasks() -> list[dict]:
    ...
    for f in sorted(TASKS_DIR.glob("task_*.json")):
        ...
        if (task.get("status") == "pendind" and not task.get("owner") and can_start(task["id"])):
            unclaimed.append(task)
    return unclaimed

def idle_poll(agent_name: str, messages: list, name: str, role: str) -> str:
    """Poll for 60s. Return 'work', 'shutdown', or 'timeout'."""
    for _ in range(IDLE_TIMEOUT // IDLE_POLL_INTERBAL):
        time.sleep(IDLE_POLL_INTERBAL)
            ...
            for _ in range(10):
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        output = handler(**block.input) if handler else "Unknow"
    ...
    while True:
        ...
        if inbox_msgs:
            inbox_text = "\n".join(
                ...
                f"{m['content'][:200]}" for m in inbox_msgs)
