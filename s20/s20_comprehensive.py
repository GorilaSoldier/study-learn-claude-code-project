...
def run_git(args: list[str]) -> tuple[bool, str]:
    try:
        ...
        return r.returncode == 0, out[:5000] if out else "(no output)" if out else "(no output)"
...
def remove_worktree(name: str, discard_changes: bool = False) -> str:  # discard_changes : 是否强制删除，即使有未提交的变更
    err = validate_worktree_name(name)
    if err:
        return err
    path = WORKTREES_DIR / name
...
def load_skills(name: str) -> str:
    ...
    if not skill:
        ...
        return f"Skill not found: {name}. Avaliable: {available}"
...
def _normalize_todos(todos):
    if isinstance(todos, str):
        try:
            ...
            try:
                todos = ast.literal_eval(todos) # literal_eval是什么用法？
...
def idle_poll(agent_name: str, messages: list, name: str, worktree_context: dict | None = None) -> str:
        ...
        if unclaimed:
            ...
            if "Claimed" in result:
                ...
                if task_data.get("worktree"):
                    wt_path = WORKDIR / task_data["worktree"]
                    wt_info = f"\nwork directory: {wt_path}"
                    if worktree_context is not None:          # 为什么context不为空要重新加入路径？
                        worktree_context["path"] = str(wt_path)
                messages.append({"role": "user", "content": f"<auto-claimed>Task {task_data['id']}: "
...
def spawn_teammate_thread(name: str, role: str, prompt: str) -> str:
    ...
    protocol_ctx = {"waiting_plan": None}  # 这一行是什么作用？
    ...
    def handle_inbox_message(name: str, msg: dict, messages: list):
        ...
        if msg_type == "plan_approval_response":
            ...
            messages.append({"role": "user", "content": "[Plan approved]" if approve 
    ...
    def run():
        ...
        while True:
            ...
            for _ in range(10):
                ...
                if not has_tool_use(response.conteant):
                    break
                results = []
                for block in response.content:
                    if block.type == "tool_use":
                        if block.name == "submit_plan":
                            ...
                            match = re.search(r"\((req_\d+)\)", output) # 此处搜索的作用是?
...
def _teammate_submit_plan(from_name: str, plan: str) -> str:
    ...
    # 此处plan_approval的意思是获取plan支持吗？
    pending_requests[req_id] = ProtocolState(
        request_id=req_id, type="plan_approval",
        sender=from_name, target="lead",
        status="pending", payload=plan
    )
    # lead通过req_id去查protocolstate吗？
    BUS.send(from_name, "lead", plan, "plan_approval",
            {"request_id": req_id})
...
def run_request_shutdown(teammate: str) -> str:
    ...
    BUS.send("lead", teammate, "Shut down.", "shutdown_request",
            {"request_id": req_id})
    return f"Shutdown reqeust sent to {teammate}"
...
def run_review_plan(request_id: str, approve: bool, feedback: str = "") -> str:
    state = pending_requests.get(request_id)
    if not state:
        return f"Plan {request_id} not found"
    state.status = "approved" if approve else "rejected"
    #调用该函数后，approve或者rejected存在哪里了?
...
HOOKS = {"UsePromotSubmit": [], "PreToolUse": [],
                                   "PostToolUse": [], "Stop": []}
...
# content中的item都装什么？这里在哪里规定调用工具次数的？为什么加这种规定？
...
def extract_text(content) -> str:
    ...
    return "\n".join(
        getattr(block, "text", "")
        for block in content if getattr(block, "type", None) == "text").strip()
    ...
    for _ in range(30):
        ...
        for block in response.content:
            ...
            if blocked:
                ...
                # 有pretooluse hook 就不调用handler了吗? hook不就只是在调用handler之前做一些事情吗？
                handler = SUB_HANDLERS.get(block.name)
                # block.input都是什么？ 为什么传name？ 看起来根本没用到
                output = call_tool_handler(handler, block.input, block.name)
                # 这里不用接受posttooluse结果吗?
...
def persist_large_output(tool_use_id: str, output: str) -> str:
    ...
    path = TOOL_RESULTS_DIR / f"{tool_use_id}.json"
    if not path.exists():
...
def tool_result_budget(messages: list, max_bytes: int = 200_000) -> list:
    ...
    for _, block in sorted(blocks, key=lambda pair: len(str(pair[1].get("content", ""))), reverse=True):
        ...
        block["content"] = persist_large_output((block["tool_use_id"], "unknown"), text)
        total = sum(len(str(b.get("content", ""))) for _, b in blocks)
    return messages
...
def reactive_compact(messages: list) -> list:
    ...
    # 这在干什么？
    if (tail_start > 0 and tail_start < len(messages) and is_tool_result_message(messages[tail_start])
       and message_has_tool_use(messages[tail_start - 1])):
        tail_start -= 1
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "ratelimit" in name or "429" in msg:
                ...
                time.sleep(delay)
                continue
            if "overload" in name or "529" in msg or "overloaded" in msg:
...
# 已经有cron_field_matches，这里为什么又写一次？
...
def run_shedule_cron(cron: str, prompt: str, recurring: bool = True, durable: bool = True) -> str:
...
# 为什么叫disallowed? 这样获得不是allowed的吗？
